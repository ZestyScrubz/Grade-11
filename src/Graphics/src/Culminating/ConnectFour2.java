...
public class ConnectFour2 implements ActionListener {
	...
	void introButton() {
		...
		while (true) {
			...
			if (intro.getKeyCode() == 27) {
				timer.start(); 
				return;	 //the ESC button was pressed
			}
	...
	void countDown() {
		...
		if (time2 == 8) {
			gc.setFont(fontMain);
			gc.drawString("Player 1's turn (Red)", timerX - 80, timerY - 180);
		}
		if (time1 == 3) {
			gc.setFont(fontTime);
		...
		if (time1 == 8) {
			gc.setFont(fontMain);
			gc.drawString("Player 2's turn (Blue)", timerX - 80, timerY - 180);
		}
		if (time2 == 3) {
			gc.setFont(fontTime);
