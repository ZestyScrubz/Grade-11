...
public class ConnectFour6 implements ActionListener {
	...
	static final int TIMERSPEED = 200;
	double time1 = 7.0, time2 = 7.0;
	...
	ConnectFour6() {
		...
		introButton(); // switch graphics console

		// game loop
		while(gameover && !tie) { 
			if(gc.getKeyCode() == 'Q') break;
	...
	void introButton() {
		...
		intro.drawString("You will have 7 seconds to play a piece.", 125, 150);
		intro.drawString("You might recieve an 'anvil' piece at random which will destroy", 20, 200);
		...
		intro.drawString("*Press 'q' anytime to leave*", 190, 580);
	...
	void infoButton() {
		...
		info.drawString("Place the piece and it will destroy all piece under it.", 95, 150);
	...
	void drawGraphics() {
		...
		boxW = winW/gridC; // width of box
		boxH = winH/gridR; // hight of box
		...
		synchronized(gc) {
			...
			for(int i = 0; i <= gridC; i++) {
				...
				gc.drawLine(i*boxW, 1, i*boxW, winH); // to make the line look thicker
				gc.drawLine(i*boxW, 2, i*boxW, winH);
				gc.drawLine(0, i*boxH, winW, i*boxH);
				gc.drawLine(1, i*boxH, winW, i*boxH);
				gc.drawLine(2, i*boxH, winW, i*boxH);
			}
	...
	void anvil() {
		Random r = new Random();
		int anvil = r.nextInt(15) + 1;	 // random number from 1-15
		System.out.println(anvil);
		if (anvil > 14) { // if the number is 15 give player anvil
	...
	void checkWin() {
		...
		for (int r = 3; r < gridR; r++){ // you can't make a ascending diagonal three row down and three columns across 
			for (int c = 0; c < gridC-3; c++){ // skipping 3
				if (board[r][c] == P1 && board[r-1][c+1] == P1 && board[r-2][c+2] == P1 && board[r-3][c+3] == P1) { // check one left, one down. check two left two down. and so on until 4 to connect four
					gc.showDialog("Player 1 wins!", " ");
					gameover = false;
				} else if (board[r][c] == P2 && board[r-1][c+1] == P2 && board[r-2][c+2] == P2 && board[r-3][c+3] == P2) { // same concept but check for second players piece
					gc.showDialog("Player 2 wins!", " ");
					gameover = false;
				}
			}
		...
		for (int r = 3; r < gridR; r++){ // same concept as the ascending diagonal
			for (int c = 3; c < gridC; c++){
				if (board[r][c] == P1 && board[r-1][c-1] == P1 && board[r-2][c-2] == P1 && board[r-3][c-3] == P1) { // check one right, one down. check two right, two down. and so on until 4 to connect four
					gc.showDialog("Player 1 wins!", " ");
					gameover = false;
				} else if (board[r][c] == P2 && board[r-1][c-1] == P2 && board[r-2][c-2] == P2 && board[r-3][c-3] == P2) { // same concept but check for second players piece
					gc.showDialog("Player 2 wins!", " ");
	...
	void countDown() {
		...
		if (time2 == 8.0 && smash) {
			...
			gc.drawString("YOU HAVE ANVIL", timerX - 73, timerY - 205);
		} else if (time2 == 8.0 && !smash) {
			gc.setFont(fontMain);
			gc.drawString("Player 1's turn (Red)", timerX - 90, timerY - 180);
		}
		
		if ((int)time1 == 3) { // if the timer is any second in 3 seconds display
			gc.setFont(fontTime);
			gc.drawString("" + (int)time1, timerX + 5, timerY);
		}
		if ((int)time1 == 2) {
			gc.setFont(fontTime);
			gc.drawString("" + (int)time1, timerX + 10, timerY);
		}

		if ((int)time1 == 1) {
			gc.setFont(fontTime);
			gc.drawString("" + (int)time1, timerX, timerY);
		}

		if ((int)time1 == 8 && smash) {
			gc.setFont(fontMain);
			gc.drawString("Player 2's turn (Blue)", timerX - 90, timerY - 180);
			gc.drawString("YOU HAVE ANVIL", timerX - 73, timerY - 205);
		} else if ((int)time1 == 8 && !smash) {
			gc.setFont(fontMain);
			gc.drawString("Player 2's turn (Blue)", timerX - 90, timerY - 180);
		}
		...
		if ((int)time2 == 3) {
			gc.setFont(fontTime);
			gc.drawString("" + (int)time2, timerX + 5, timerY);
		}
		if ((int)time2 == 2) {
			gc.setFont(fontTime);
			gc.drawString("" + (int)time2, timerX + 10, timerY);
		}
		if ((int)time2 == 1) {
			gc.setFont(fontTime);
			gc.drawString("" + (int)time2, timerX, timerY);
		} 
	}
	
	@Override
	public void actionPerformed(ActionEvent ev) {
		time1 -= TIMERSPEED/1000.0; // every seconds
		time2 -= TIMERSPEED/1000.0; 
